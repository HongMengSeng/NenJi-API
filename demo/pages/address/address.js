    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          // 跳转到新增地址页，并预填当前位置
          wx.navigateTo({
            url: `/pages/address-edit/address-edit?province=${encodeURIComponent(location.province)}&city=${encodeURIComponent(location.city)}&district=${encodeURIComponent(location.district)}&street=${encodeURIComponent(location.street + location.streetNumber)}&lat=${location.latitude}&lng=${location.longitude}`
          });
        }
      }
    });
